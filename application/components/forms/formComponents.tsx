...
interface GenericFormState {
	...
	fieldsState?: Record<string, string | string[]>;
	message?: string | null;
}
...
type GenericFormState2<T extends string = string> = {
	errors?: Partial<Record<T, string | string[]>>;
	fieldsState?: Partial<Record<T, string | string[]>>;
	message?: string | null;
};
		...
		<div className={"grid gap-2"}>
			...
			<div className={"relative"}>
				<Input
					type={inputType}
					className={cn(
						"bg-black/10 rounded-2xl w-full outline-none focus:border-primary focus:faint-glow-secondary",
						className,
					)}
			...
			<div id={`${fieldName}-error`}>
				{!isPending &&
					(Array.isArray(error) ? (
						error.map((msg) => (
							<p key={msg} className="ml-2 text-sm text-red-500">
								{msg}
							</p>
						))
					) : typeof error === "string" ? (
	...
	return (
		<div className={cn("grid gap-2")}>
			...
			<div className={"relative"}>
				<Textarea
					className={cn(
						"bg-black/10 z-20 w-full outline-none focus:border-primary focus:faint-glow-secondary",
						className,
					)}
					placeholder={placeholder}
					name={fieldName}
