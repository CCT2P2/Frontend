...
export async function createAccount(
	...
	if (!response.ok) {
		let errorMessage = `Error creating account: ${response.statusText}`;
		try {
			const errorBody = await response.text();
			if (errorBody) {
				errorMessage = `Error creating account: ${errorBody}`;
			}
		} catch (err) {
			errorMessage = `Error creating account: unknown`;
		}
		return generateFormResponse(formData, validatedFields, errorMessage);
	}
